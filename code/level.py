...
class Level:
	def __init__(self, current_level, window, create_overworld, change_coins, change_health):
		...
		self.coin_sound = pygame.mixer.Sound('../graphics/ui/coin.mp3')
		self.hurt_sound = pygame.mixer.Sound('../graphics/ui/hurt.wav')
		self.gameover_sound = pygame.mixer.Sound('../graphics/ui/gameover.wav')
		self.kill_sound = pygame.mixer.Sound('../graphics/ui/kill.wav')
		...
		for row_index, row in enumerate(layout):
			for col_index, val in enumerate(row):
				if val != "-1":
					...
					if type == 'coins':
						if val == '1': sprite = Coin(tile_size,x,y,'../graphics/coins/gold', 5)
						if val == '0': sprite = Coin(tile_size,x,y,'../graphics/coins/silver', 1)
	...
	def check_death(self):
		if self.player.sprite.rect.top>win_h:
	...
	def check_enemy_collide(self):
		enemy_collis = pygame.sprite.spritecollide(self.player.sprite, self.enemies_sprites, False)

		if enemy_collis:
			for enemy in enemy_collis:
	...
	def run(self):
		...
		self.explosion_sprites.update(self.worldshift)
